...
def split_dataset(X, y, test_size=0.2, random_state=42):
    """Split data once so training and evaluation are separated."""
...
def run_elasticnet_baseline(X_train, y_train, X_test, y_test):
    """Evaluate ElasticNet alone as a pure scoring model on held-out test data."""
    ...
    metrics = eval_performance(y_test, y_hat)
    metrics['top10_hit_rate'] = get_topk_hit_rate(y_test, scores, 0.10)
    metrics['top20_hit_rate'] = get_topk_hit_rate(y_test, scores, 0.20)
    return metrics


def run_ea(X_train, y_train, X_test, y_test, **kwargs):
    """Run the full EA on train data and evaluate best evolved rule on test data."""
    feature_space = range(X_train.shape[1])
    results = evolve(X_train, y_train, feature_space=feature_space, **kwargs)

    best = results['best_individual']
    y_hat = apply_hypothesis(best, X_test)
    metrics = eval_performance(y_test, y_hat)

    risk_scores = predict_risk(results['model'], X_test)
    metrics['top10_hit_rate'] = get_topk_hit_rate(y_test, risk_scores, 0.10)
    metrics['top20_hit_rate'] = get_topk_hit_rate(y_test, risk_scores, 0.20)
...
def exp1_fitness_curves():
    ...
    for ax, (name, path) in zip(axes, DATASETS.items()):
        ...
        res = run_ea(X_train, y_train, X_test, y_test, **EA_PARAMS)
...
def exp2_ea_vs_baseline():
    ...
    for name, path in DATASETS.items():
        ...
        en = run_elasticnet_baseline(X_train, y_train, X_test, y_test)
        ea = run_ea(X_train, y_train, X_test, y_test, **EA_PARAMS)
...
# Shows how a key EA parameter affects performance
...
def exp3_mutation_sensitivity():
    ...
    for rate in mutation_rates:
        ...
        res = run_ea(X_train, y_train, X_test, y_test, **params)
...
def exp4_topk_hit_rates():
    ...
    for ax, (name, path), color in zip(axes, DATASETS.items(), colors):
        ...
        res = run_ea(X_train, y_train, X_test, y_test, **EA_PARAMS)
        risk_scores = predict_risk(res['model'], X_test)

        hit_rates = [get_topk_hit_rate(y_test, risk_scores, k) for k in k_values]
